...
import static com.arsylk.mammonsmite.utils.Define.MODELS_DIRECTORY;
...
public class DCSwapper {
    ...
    private File getOutputPath() {
        //sure but why?
        File swapOutput = new File(MODELS_DIRECTORY, toL2D.getOutput().getName()+"_swap");
        int tries = 0;
        while(swapOutput.exists()) {
            tries += 1;
            swapOutput = new File(MODELS_DIRECTORY, toL2D.getOutput().getName()+"_swap_"+tries);
        }
        lastFolder = swapOutput;
        return swapOutput;
    }
